...
def start_background_downloader():
    def fetch_all_silently():
        ...
        end_d = datetime.now(); start_d = end_d - timedelta(days=365*10)
        for sym in SYMBOLS:
            ...
            try:
                ...
                if not df.empty: df.to_csv(tmp_path); os.replace(tmp_path, file_path)
            except Exception:
                try:
                    df = fetch_yfinance(sym, start=start_d, end=end_d, freq="5m")
                    if not df.empty: df.to_csv(tmp_path); os.replace(tmp_path, file_path)
                except Exception: pass
    threading.Thread(target=fetch_all_silently, daemon=True).start()
    return True
...
event_filters    = st.sidebar.multiselect("High-Impact Macro Events",
    ["NFP","FOMC","CPI","PCE","Triple Witching","End-of-Month"], default=[])
calendar_filters = st.sidebar.multiselect("CME Calendar Anomalies",
    ["Market Holidays","Half-Day Sessions"], default=[])
...
st.sidebar.markdown("---")
show_coverage = st.sidebar.checkbox("Show bar coverage diagnostic", value=False)


# ─────────────────────────────────────────────
# DATA FETCHING
# ─────────────────────────────────────────────
end_dt = datetime.now(); start_dt = end_dt - timedelta(days=365*10)

@st.cache_data(ttl=timedelta(hours=4), show_spinner=False)
def get_raw_data(sym):
    local_file = os.path.join(CACHE_DIR, f"{sym}_local_cache.csv")
    if os.path.exists(local_file):
        return pd.read_csv(local_file, index_col=0, parse_dates=True), "Local Database"
    ts = TradeStationFetcher()
    try:
        df = ts.get_intraday_bars(sym, start=start_dt, end=end_dt, interval="5")
        df.to_csv(local_file); return df, "TradeStation"
    except Exception:
        try:
            df = fetch_yfinance(sym, start=start_dt, end=end_dt, freq="5m")
            df.to_csv(local_file); return df, "yfinance"
...
def calculate_seasonals(df_raw, years_lookback, filters, tz_setting,
    ...
    cutoff = pd.to_datetime(
        datetime.strptime(end_reference, "%Y-%m-%d") - timedelta(days=365*years_lookback), utc=True)
    df = df_raw.copy()
    if df.index.tz is None: df.index = df.index.tz_localize("UTC")
    df = df[df.index >= cutoff]
    df.index = df.index.tz_convert(
        "Europe/Zurich" if "Zürich" in tz_setting else "America/New_York")
    for evt in filters:
...
if raw_df_full.empty:
    st.error("❌ Failed to fetch data."); st.stop()
...
if PathData.empty:
    st.warning("Not enough data matches the selected filters."); st.stop()
...
st.caption(f"Data: {source}  •  Filters: {' | '.join(active) if active else 'None'}")
...
st.caption(f"Win Rate ≥ {min_win_rate}% | PF ≥ {min_profit_factor} | p ≤ {max_p_value} | N ≥ {min_obs}")
...
if not edge_df.empty:
    display_cols = [c for c in
        ["label","avg","median","win_rate","profit_factor","t_stat","p_value","sharpe","sortino","kelly","count"]
        if c in edge_df.columns]
    fmt = {"avg":"{:.3f}%","median":"{:.3f}%","win_rate":"{:.1f}%","profit_factor":"{:.2f}",
           "t_stat":"{:.2f}","p_value":"{:.4f}","sharpe":"{:.2f}","sortino":"{:.2f}",
           "kelly":"{:.3f}","count":"{:.0f}"}
    st.dataframe(edge_df[display_cols].head(10).style.format(
        {k:v for k,v in fmt.items() if k in display_cols}), use_container_width=True)
else:
    st.warning("No slots pass the selected thresholds. Try relaxing the filters.")
...
if not edge_df.empty:
    ...
    with col1: st.success(f"**Best Entry Slot:**\n### {top.get('label','—')}")
    with col2: st.info(f"**Avg Return:**\n### {top['avg']:.3f}%")
    with col3: st.warning(f"**Win Rate (N={int(top['count'])}):**\n### {top['win_rate']:.1f}%")
    with col4:
        kstr = f"{top['kelly']:.1%}" if not np.isnan(top.get("kelly", float("nan"))) else "N/A"
        st.info(f"**Kelly Size:**\n### {kstr}")
    sharpe = top.get("sharpe", float("nan"))
    if not np.isnan(sharpe):
        st.caption(f"Sharpe: **{sharpe:.2f}** | PF: **{top.get('profit_factor',float('nan')):.2f}** | "
...
# ─────────────────────────────────────────────
# SECTION 3: AVERAGE PATH  (no percentile bands)
# ─────────────────────────────────────────────
st.markdown("---")
st.subheader(f"Cumulative Weekly Path — Mean & Median ({tz_mode})")
...
fig.add_trace(go.Scatter(
    ...
    hovertemplate="<b>%{text}</b><br>Avg: %{y:.3f}%<br>Win Rate: %{customdata[0]:.1f}%<br>"
                  "t-stat: %{customdata[1]:.2f}<br>N: %{customdata[2]:.0f}<br>"
                  "PF: %{customdata[3]:.2f}<extra></extra>"
))

# Median path (robust alternative)
...
# Day boundaries
tick_vals = [24, 48, 72, 96, 120]; tick_text = ["Mon","Tue","Wed","Thu","Fri"]
...
fig.update_layout(
    ...
    yaxis_title="% move vs weekly open", hovermode="x unified",
    margin=dict(l=0, r=0, t=30, b=0))
st.plotly_chart(fig, use_container_width=True)


# ─────────────────────────────────────────────
# SECTION 3b: COVERAGE DIAGNOSTIC (optional)
# ─────────────────────────────────────────────
if show_coverage:
    st.subheader("🔬 Bar Coverage Diagnostic")
    st.caption(
        "Bars per 15-min time slot across the week. "
        "Red = sparse zone (< 80% of typical N) — "
        "these slots are most prone to averaging artifacts."
    )
    tz_conv = "Europe/Zurich" if "Zürich" in tz_mode else "America/New_York"
    df_tz = raw_df_full.copy()
    if df_tz.index.tz is None: df_tz.index = df_tz.index.tz_localize("UTC")
    df_tz.index = df_tz.index.tz_convert(tz_conv)

    cov = compute_coverage_profile(df_tz, freq="15min", symbol=symbol)
    if not cov.empty:
        fig_cov = go.Figure()
        colors = ["#d62728" if s else "#1f77b4" for s in cov["sparse"]]
        fig_cov.add_trace(go.Bar(
            x=cov["ct"], y=cov["n"],
            marker_color=colors, name="Bar count",
            text=cov["label"], hovertemplate="<b>%{text}</b><br>N: %{y}<extra></extra>"
        ))
        fig_cov.add_hline(
            y=cov["n"].mode()[0] * 0.8, line_dash="dash", line_color="orange",
            annotation_text="80% threshold"
        )
        for x_val in tick_vals:
            fig_cov.add_vline(x=x_val, line_width=1, line_dash="dash", line_color="gray")
        fig_cov.update_layout(
            xaxis=dict(title="", tickmode="array", tickvals=tick_vals, ticktext=tick_text),
            yaxis_title="Bar count per slot",
            margin=dict(l=0, r=0, t=30, b=0)
        )
        st.plotly_chart(fig_cov, use_container_width=True)

        sparse_slots = cov[cov["sparse"]]
        if not sparse_slots.empty:
            st.warning(
                f"⚠️ {len(sparse_slots)} sparse time slots detected. "
                f"Worst: **{sparse_slots.iloc[sparse_slots['n'].argmin()]['label']}** "
                f"(N={sparse_slots['n'].min()}). "
                "These are automatically excluded by the Min Observations filter."
            )
...
st.caption("Grey = no data / excluded by session filter. Not filled with 0%.")
max_hm_val = HeatmapData.abs().max().max()
fig_hm = px.imshow(HeatmapData, labels=dict(x="Time",y="Day",color="% move"),
    color_continuous_scale="RdYlGn", range_color=[-max_hm_val, max_hm_val], aspect="auto")
...
with col_chart1:
    ...
    if "win_rate" in DailyPerf.columns:
        ...
        fig_win.add_hline(y=50, line_dash="dash", line_color="red", annotation_text="50%")
        fig_win.add_hline(y=55, line_dash="dot", line_color="orange", annotation_text="55% edge")
        fig_win.update_layout(yaxis_title="Win Rate (%)", margin=dict(l=0,r=0,t=30,b=0))
        st.plotly_chart(fig_win, use_container_width=True)
    disp_cols = [c for c in ["avg","win_rate","profit_factor","sharpe","sortino","kelly","count"]
                 if c in DailyPerf.columns]
    if disp_cols:
        st.dataframe(DailyPerf[disp_cols].style.format({
...
with col_chart2:
    ...
    if not HourlyVol.empty:
        fig_vol = go.Figure()
        fig_vol.add_trace(go.Bar(x=HourlyVol["time_label"], y=HourlyVol["vol"],
            name="Mean ATR %", marker_color="#ff7f0e"))
        if "vol_p90" in HourlyVol.columns:
            fig_vol.add_trace(go.Scatter(x=HourlyVol["time_label"], y=HourlyVol["vol_p90"],
